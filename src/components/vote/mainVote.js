...
const VotePage = () => {
    ...
    if (!isVoted) {
      try {
        ...
        if (place) {
          ...
          const voteData = { 
            category: mappedCategory, // Sử dụng danh mục đã ánh xạ
            placeId: place.placeId,
            year: currentYear // Kèm theo năm hiện tại
          };
  ...
  const handleSearch = e => {
    ...
    setSearchTerm(value);
    handleSearchName(value); // Gọi hàm tìm kiếm khi người dùng nhập
  };
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
    {selectedLocations.length > 0 ? (
      selectedLocations.slice(0, 12).map(location => {
        // Lấy giá trị tương ứng từ categoryMapping
        ...
        return (
          <div key={location.placeId} className="relative flex flex-col items-center p-4 transition-transform transform hover:scale-105 group">
            <div className="relative w-40 h-40 rounded-full bg-green-200 flex items-center justify-center border-4 border-gray-300">
              <div className="w-36 h-36 rounded-full bg-blue-100 flex items-center justify-center border-2 border-gray-400">
            ...
            <div className="flex flex-col items-center mt-2 text-center">
              <span className="font-semibold">{location.name}</span> {/* Tên địa điểm */}
              <button
                onClick={() => handleVote(categoryKey, location)} // Gọi hàm handleVote với categoryKey
                ...
                disabled={isVoted} // Vô hiệu hóa nút nếu đã bình chọn
              >
      ...
      <p className="text-center">Không có địa điểm nào.</p>
    )}
  </div>
</div>
